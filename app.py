...
def normalize_hand_code(raw_value, default="") -> str:
    ...
    if txt in {"L", "R", "S"}:
        return txt
    if txt in {"LEFT", "LEFTY", "LHB", "LHP"}:
        return "L"
    if txt in {"RIGHT", "RIGHTY", "RHB", "RHP"}:
        return "R"
    if txt in {"SWITCH", "SH", "SHB"}:
        return "S"
    return default


@st.cache_data(ttl=21600)
def fetch_true_pitcher_pitch_mix(pitcher_id, year: int):
    """Return actual Statcast pitch usage for this pitcher; never invent pitch types."""
    try:
        pid = int(pitcher_id)
    except Exception:
        return {}
    ...
    pitch_series = df["pitch_type"].dropna().astype(str).str.strip().str.upper()
    pitch_series = pitch_series[pitch_series.ne("")]
    if pitch_series.empty:
        return {}

    counts = pitch_series.value_counts()
    ...
    for pitch_code, count in counts.items():
        usage = round((int(count) / total) * 100.0, 1)
        if usage >= 1.0:
            mix[str(pitch_code)] = usage

    return dict(sorted(mix.items(), key=lambda x: x[1], reverse=True))


def serialize_pitch_mix(pitch_mix: dict) -> str:
    if not pitch_mix:
        return "No verified pitch mix"
    return " | ".join([f"{str(k).upper()} {safe_float(v, 0.0):.1f}%" for k, v in sorted(pitch_mix.items(), key=lambda x: safe_float(x[1], 0.0), reverse=True)])
...
def build_hitter_metrics(
    ...
    hitter_data = hitter_stats_map.get(player_id, {}) if isinstance(hitter_stats_map, dict) else {}
    pitcher_data = pitcher_stats_map.get(opp_pitcher_id, {}) if isinstance(pitcher_stats_map, dict) else {}
    bats = normalize_hand_code(hitter_data.get("bat_side"), "")
    pitcher_throws = normalize_hand_code(pitcher_data.get("pitch_hand"), "")
    ...
    pitch_mix_example = fetch_true_pitcher_pitch_mix(opp_pitcher_id, CURRENT_SEASON)
    ...
    return {
        ...
        "Relevant Pitch Mix": serialize_pitch_mix(pitch_mix_example),
...
def render_player_card(row: pd.Series, rank_override=None):
    ...
    st.caption(f"vs {pitcher} | Batter: {_display_hand(row.get('Bats'), 'batter')} | Pitcher: {_display_hand(row.get('Pitcher Throws'), 'pitcher')}")
