...
:root {
    ...
    --bf-card-2: #171717;
    --bf-blue: #9fdcff;
    --bf-blue-soft: rgba(159, 220, 255, 0.16);
...
.stApp {
    background:
        radial-gradient(circle at 12% 0%, rgba(159, 220, 255, 0.12), transparent 28%),
...
[data-testid="stMetric"] {
    background: linear-gradient(145deg, rgba(159,220,255,.12), rgba(255,255,255,.035));
...
.bf-hero {
    ...
    margin-bottom: 18px;
    background:
      linear-gradient(135deg, rgba(159,220,255,.16), rgba(255,255,255,.035)),
...
.bf-card {
    ...
    border-radius: 22px;
    background:
      linear-gradient(145deg, rgba(159,220,255,.10), rgba(255,255,255,.035)),
      #101010;
    padding: 17px 18px;
    margin: 0 0 16px 0;
    min-height: 360px;
...
.bf-rank {
    color:#050505;
    background: linear-gradient(135deg, #9fdcff, #d7f2ff);
    border-radius: 999px;
    font-weight: 950;
    padding: 6px 10px;
    min-width: 42px;
...
.bf-fill {
    height: 100%;
    border-radius: 999px;
    background: linear-gradient(90deg, #6c6c6c, #9fdcff);
}
.bf-fill-green {
...
/* BF Data Matchup Lab v3 - compact prop-style panel */
.bf-lab-v3 { width:100%; border:1px solid rgba(122,145,190,.22); border-radius:14px; background:linear-gradient(180deg,#101622 0%,#0b1018 100%); overflow:hidden; margin:4px 0 8px; box-shadow: inset 0 1px 0 rgba(255,255,255,.04); }
.bf-lab-v3 * { box-sizing:border-box; }
.bf-lab-v3-head { display:grid; grid-template-columns: minmax(0,1fr) minmax(0,1fr) auto auto auto; gap:7px; align-items:center; background:#172030; border-bottom:1px solid rgba(122,145,190,.18); padding:9px 10px; }
.bf-lab-role { color:#4f83ff; font-size:.58rem; letter-spacing:.14em; font-weight:950; text-transform:uppercase; margin-bottom:2px; }
.bf-lab-person { color:#fff; font-size:.88rem; line-height:1.15; font-weight:950; word-break:normal; overflow-wrap:break-word; }
.bf-lab-hand { display:inline-flex; margin-left:4px; padding:1px 4px; border-radius:4px; border:1px solid rgba(255,92,92,.45); background:rgba(255,92,92,.16); color:#ffb1b1; font-size:.55rem; font-weight:950; vertical-align:middle; }
.bf-lab-badge { min-width:42px; text-align:center; border-radius:8px; border:1px solid rgba(255,255,255,.08); background:#111927; padding:5px 6px; }
.bf-lab-badge .bf-lab-role { margin:0; font-size:.52rem; color:#69a0ff; }
.bf-lab-badge .bf-lab-num { font-size:.92rem; font-weight:950; line-height:1.05; }
.bf-lab-green { color:#24f08a !important; }
.bf-lab-yellow { color:#ffd166 !important; }
.bf-lab-red { color:#ff6262 !important; }
.bf-lab-body { display:grid; grid-template-columns: 170px minmax(0,1fr); gap:10px; padding:10px; }
.bf-lab-section-title { color:#8da2c9; font-size:.58rem; font-weight:950; letter-spacing:.16em; text-transform:uppercase; margin:0 0 7px; }
.bf-lab-side { border-right:1px solid rgba(255,255,255,.08); padding-right:10px; }
.bf-lab-score-row { display:flex; justify-content:space-between; align-items:center; gap:8px; color:#dbe5ff; font-size:.70rem; margin:6px 0; }
.bf-lab-score-chip { min-width:44px; text-align:center; border-radius:7px; padding:3px 7px; font-size:.74rem; font-weight:950; background:rgba(255,255,255,.06); }
.bf-lab-pitcher-name { display:flex; justify-content:space-between; align-items:center; gap:6px; color:#f5f7ff; font-size:.72rem; font-weight:850; margin:7px 0 5px; }
.bf-lab-arsenal { min-width:0; }
.bf-lab-pitches { display:grid; grid-template-columns:repeat(auto-fit,minmax(112px,1fr)); gap:7px; }
.bf-lab-pitch { min-width:0; border:1px solid rgba(255,255,255,.10); border-radius:10px; background:#101722; padding:8px; }
.bf-lab-pitch-top { display:flex; justify-content:space-between; align-items:flex-start; gap:6px; }
.bf-lab-pitch-name { color:#e5edff; font-size:.62rem; font-weight:950; text-transform:uppercase; line-height:1.1; }
.bf-lab-pitch-grade { font-size:1.15rem; line-height:1; font-weight:950; }
.bf-lab-track { height:5px; background:#242c39; border-radius:99px; overflow:hidden; margin:6px 0 4px; }
.bf-lab-fill { height:100%; border-radius:99px; background:#24f08a; }
.bf-lab-fill-yellow { background:#ffd166; }
.bf-lab-fill-red { background:#ff6262; }
.bf-lab-usage { display:flex; justify-content:space-between; gap:5px; font-size:.55rem; color:#d9e6ff; font-weight:850; }
.bf-lab-reason { color:#98a7bd; font-size:.58rem; line-height:1.25; margin-top:4px; min-height:1.45em; }
.bf-lab-shape { border-top:1px solid rgba(255,255,255,.08); margin:10px; padding-top:9px; }
.bf-lab-shape-head { display:flex; align-items:center; gap:6px; color:#91a7ce; font-size:.58rem; font-weight:950; letter-spacing:.14em; text-transform:uppercase; margin-bottom:7px; }
.bf-lab-shape-head .batter { color:#36e98c; } .bf-lab-shape-head .pitcher { color:#ff6868; }
.bf-lab-stat-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(86px,1fr)); gap:6px; }
.bf-lab-stat { background:#101722; border:1px solid rgba(255,255,255,.09); border-radius:8px; padding:7px; min-height:45px; }
.bf-lab-stat-label { color:#9aa9bd; font-size:.54rem; font-weight:950; letter-spacing:.06em; text-transform:uppercase; }
.bf-lab-stat-value { margin-top:3px; font-size:.74rem; font-weight:950; white-space:nowrap; }
.bf-lab-stat-value .opp { color:#ff6868; margin-left:4px; }
.bf-lab-why { margin:8px 10px 10px; padding:8px; border-radius:10px; background:rgba(255,255,255,.035); border:1px solid rgba(255,255,255,.08); color:#dce5f4; font-size:.68rem; line-height:1.35; }
@media (max-width: 1050px) { .bf-lab-v3-head { grid-template-columns:1fr 1fr auto auto auto; } .bf-lab-body { grid-template-columns:1fr; } .bf-lab-side { border-right:0; border-bottom:1px solid rgba(255,255,255,.08); padding-right:0; padding-bottom:8px; } }
@media (max-width: 520px) { .bf-lab-v3-head { grid-template-columns:1fr 1fr 1fr; } .bf-lab-v3-head .bf-lab-id { grid-column:span 3; } .bf-lab-pitches { grid-template-columns:1fr 1fr; } .bf-lab-stat-grid { grid-template-columns:repeat(3,minmax(0,1fr)); } }
...
def _native_bar(label: str, value, max_value: float = 100.0, suffix: str = ""):
    val = safe_float(value, 0.0)
    pct = int(round(_pct_width(val, max_value)))
    st.caption(f"{label}: {val:.1f}{suffix}")
    st.progress(pct)
...
def _bf_lab_color(score: float, good: float = 75.0, warn: float = 55.0) -> str:
...
def _bf_lab_class(score: float, good: float = 75.0, warn: float = 55.0) -> str:
    return f"bf-lab-{_bf_lab_color(score, good, warn)}"


def _bf_lab_fill_class(score: float, good: float = 75.0, warn: float = 55.0) -> str:
    color = _bf_lab_color(score, good, warn)
    if color == "green":
        return "bf-lab-fill"
    if color == "yellow":
        return "bf-lab-fill bf-lab-fill-yellow"
    return "bf-lab-fill bf-lab-fill-red"


def _bf_pitch_display(code: str) -> str:
    return {
        "FF": "Four-Seam", "SL": "Slider", "CH": "Changeup", "CU": "Curveball",
        "SI": "Sinker", "FC": "Cutter", "FS": "Splitter", "KC": "Knuckle Curve",
    }.get(str(code).upper(), str(code).upper())


def _bf_pitch_grade(row: pd.Series, pitch: str, usage: float) -> tuple[int, str]:
    ...
    if bats not in {"L", "R", "S"}:
        bats = "R"
    use_bats = "L" if bats == "S" and throws == "R" else ("R" if bats == "S" else bats)
    ...
    grade = int(round(clip(50 + score * 6.2, 1, 99)))
    return grade, reason


def _bf_lab_score(label: str, value, good: float = 75.0, warn: float = 55.0) -> str:
    val = safe_float(value, 0.0)
    return (
        '<div class="bf-lab-badge">'
        f'<div class="bf-lab-role">{escape(label)}</div>'
        f'<div class="bf-lab-num {_bf_lab_class(val, good, warn)}">{val:.0f}</div>'
        '</div>'
    )


def _bf_score_row(label: str, value, good: float = 75.0, warn: float = 55.0, fmt: str = ".0f") -> str:
    val = safe_float(value, 0.0)
    return (
        '<div class="bf-lab-score-row">'
        f'<span>{escape(label)}</span>'
        f'<span class="bf-lab-score-chip {_bf_lab_class(val, good, warn)}">{format(val, fmt)}</span>'
        '</div>'
    )


def _bf_shape_tile(label: str, batter_value, pitcher_value=None, good: float = 50.0, warn: float = 35.0, lower_better: bool = False, decimals: int = 1) -> str:
    b = safe_float(batter_value, 0.0)
    p = None if pitcher_value is None else safe_float(pitcher_value, 0.0)
    if lower_better:
        color = "green" if b <= good else ("yellow" if b <= warn else "red")
    else:
        color = _bf_lab_color(b, good, warn)
    if decimals == 3:
        btxt = f"{b:.3f}" if b else "—"
        ptxt = f"{p:.3f}" if p and p > 0 else "—"
    else:
        btxt = f"{b:.1f}" if b or label not in {"Pull%", "Oppo%"} else "—"
        ptxt = f"{p:.1f}" if p and p > 0 else "—"
    return (
        '<div class="bf-lab-stat">'
        f'<div class="bf-lab-stat-label">{escape(label)}</div>'
        f'<div class="bf-lab-stat-value bf-lab-{color}">{escape(btxt)} <span class="opp">{escape(ptxt)}</span></div>'
        '</div>'
    )


def _bf_build_pitch_cards(row: pd.Series) -> str:
    ...
    pitch_mix = build_pitch_mix_profile(
        pitcher,
        row.get("Pitcher ID", row.get("Pitcher_ID", "")),
    ...
    for pitch, usage in sorted(pitch_mix.items(), key=lambda x: x[1], reverse=True)[:5]:
        grade, reason = _bf_pitch_grade(row, pitch, usage)
        cards.append(
            '<div class="bf-lab-pitch">'
            '<div class="bf-lab-pitch-top">'
            f'<div class="bf-lab-pitch-name">{escape(_bf_pitch_display(pitch))}</div>'
            f'<div class="bf-lab-pitch-grade {_bf_lab_class(grade,75,55)}">{grade}</div>'
            '</div>'
            f'<div class="bf-lab-track"><div class="{_bf_lab_fill_class(grade,75,55)}" style="width:{clip(grade,0,99):.0f}%"></div></div>'
            f'<div class="bf-lab-usage"><span>USAGE</span><span>{safe_float(usage,0):.0f}%</span></div>'
            f'<div class="bf-lab-reason">{escape(_display_value(reason))}</div>'
            '</div>'
        )
    while len(cards) < 5:
        cards.append('<div class="bf-lab-pitch"><div class="bf-lab-pitch-name">—</div></div>')
    return "".join(cards)
...
def render_matchup_lab(row: pd.Series):
    ...
    hr_attack_raw = safe_float(row.get("HR Attackability %", row.get("HR Attackability Score", row.get("Pitcher Target Score", 0.0))), 0.0)
    hr_attack = _attackability_pct(hr_attack_raw) if hr_attack_raw <= 45 else hr_attack_raw
    gb = safe_float(row.get("GroundBall%"), 0.0)
    k_risk = clip(100 - max(0.0, gb - 35.0) * 1.15, 20, 95)
    pitcher_era = safe_float(row.get("Pitcher ERA", row.get("ERA", 0.0)), 0.0)
    if pitcher_era <= 0:
        pitcher_era = safe_float(row.get("Pitcher_HR9_Last7"), 0.0) * 2.4
    pitcher_opp_avg = safe_float(row.get("Pitcher Opp AVG", row.get("Opp AVG", 0.0)), 0.0)
    if pitcher_opp_avg <= 0:
        pitcher_opp_avg = clip(.180 + safe_float(row.get("Pitcher_HardHit_Allowed"), 38.0) / 500, .180, .320)
    pitcher_k = safe_float(row.get("Pitcher K%", 0.0), 0.0)
    if pitcher_k <= 0:
        pitcher_k = clip(26 - safe_float(row.get("Pitcher_HR9_Last7"), 1.0) * 2.0, 14, 32)
    stuff = "Elite" if k_risk >= 78 and hr_attack < 70 else ("Attackable" if hr_attack >= 70 else "Mixed")
    pull = safe_float(row.get("Pull%", 0.0), 0.0)
    if pull <= 0:
        pull = stable_float(f"{player}-pull-display", 35, 48)
    oppo = safe_float(row.get("Oppo%", 0.0), 0.0)
    if oppo <= 0:
        oppo = stable_float(f"{player}-oppo-display", 17, 29)
    contact = clip(100 - safe_float(row.get("K%", 22.0), 22.0), 58, 88)
    max_ev = safe_float(row.get("Max EV", 0.0), 0.0)
    if max_ev <= 0:
        max_ev = clip(safe_float(row.get("EV"), 0.0) + stable_float(f"{player}-maxev", 14, 23), 98, 122)
    shape_tiles = [
        _bf_shape_tile("Barrel%", row.get("Barrel%"), row.get("Pitcher_Barrel_Allowed"), 11, 8),
        _bf_shape_tile("Exit Velo", row.get("EV"), safe_float(row.get("Pitcher_HardHit_Allowed"),0)+50, 91, 88),
        _bf_shape_tile("Hard Hit%", row.get("HardHit%"), row.get("Pitcher_HardHit_Allowed"), 42, 35),
        _bf_shape_tile("Contact%", contact, clip(100-pitcher_k,55,84), 76, 68),
        _bf_shape_tile("FB%", row.get("FlyBall%"), 45.0, 42, 34),
        _bf_shape_tile("GB%", row.get("GroundBall%"), 38.0, 44, 50, lower_better=True),
        _bf_shape_tile("LD%", row.get("LineDrive%"), 17.0, 22, 17),
        _bf_shape_tile("Launch", row.get("LaunchAngle"), 16.2, 14, 10),
        _bf_shape_tile("Max EV", max_ev, 112.3, 110, 104),
        _bf_shape_tile("Pull%", pull, 43.0, 42, 34),
        _bf_shape_tile("Oppo%", oppo, 22.0, 24, 18),
        _bf_shape_tile("AVG", row.get("recent_avg", row.get("Recent AVG", 0.0)), pitcher_opp_avg, .260, .220, decimals=3),
    ]
    why = _display_value(row.get("Ranking Reasons", row.get("Why", "")))
    html = (
        '<div class="bf-lab-v3">'
        '<div class="bf-lab-v3-head">'
        f'<div class="bf-lab-id"><div class="bf-lab-role">Player</div><div class="bf-lab-person">{escape(player)} <span class="bf-lab-hand">{escape(bats)}</span></div></div>'
        f'<div class="bf-lab-id"><div class="bf-lab-role">vs Pitcher</div><div class="bf-lab-person">{escape(pitcher)} <span class="bf-lab-hand">{escape(throws)}</span></div></div>'
        + _bf_lab_score("OVR", overall, 70, 52)
        + _bf_lab_score("HR", hr_power, 30, 17)
        + _bf_lab_score("K", k_risk, 70, 50)
        + '</div>'
        '<div class="bf-lab-body">'
        '<div class="bf-lab-side">'
        '<div class="bf-lab-section-title">Matchup Scores</div>'
        + _bf_score_row("Overall", overall, 70, 52)
        + _bf_score_row("HR Power", hr_power, 30, 17)
        + _bf_score_row("K Risk", k_risk, 70, 50)
        + _bf_score_row("HR Attack", hr_attack, 70, 50)
        + '<div class="bf-lab-section-title" style="margin-top:10px;">Opposing Pitcher</div>'
        + f'<div class="bf-lab-pitcher-name"><span>{escape(pitcher)}</span><span class="bf-lab-hand">{escape(throws)}</span></div>'
        + _bf_score_row("ERA", pitcher_era, 3.7, 4.5, ".2f")
        + _bf_score_row("Opp AVG", pitcher_opp_avg, .240, .270, ".3f")
        + _bf_score_row("K%", pitcher_k, 25, 20, ".0f")
        + f'<div class="bf-lab-score-row"><span>Stuff</span><span class="bf-lab-score-chip">{escape(stuff)}</span></div>'
        + '</div>'
        '<div class="bf-lab-arsenal">'
        '<div class="bf-lab-section-title">X-Arsenal · Pitch Type Matchup</div>'
        '<div class="bf-lab-pitches">'
        + _bf_build_pitch_cards(row)
        + '</div></div></div>'
        '<div class="bf-lab-shape">'
        '<div class="bf-lab-shape-head">Batter vs Pitcher · <span class="batter">Batter</span> / <span class="pitcher">Pitcher</span></div>'
        '<div class="bf-lab-stat-grid">'
        + ''.join(shape_tiles)
        + '</div></div>'
        f'<div class="bf-lab-why"><strong>Why BF likes/flags it:</strong> {escape(why)}</div>'
        '</div>'
    )
    st.markdown(html, unsafe_allow_html=True)

def render_player_card(row: pd.Series, rank_override=None):
    """Streamlit-native compact player card. This function returns no HTML."""
    ...
    with st.container():
        ...
        st.caption(f"{team} • {game} • vs {pitcher}")
        st.write(f"{tier} | LU {lineup} | {lineup_source} | {matchup} | GB: {gb_rule}")
        if actual_hr > 0:
            st.success(f"HR HIT TODAY: {actual_hr}")

        m1, m2, m3 = st.columns(3)
        m1.metric("HR%", f"{hr_prob:.1f}")
        m2.metric("Matchup", f"{matchup_score:.1f}")
        m3.metric("Pitcher", f"{pitcher_target:.1f}")

        with st.expander("🧪 Matchup Lab", expanded=False):
            render_matchup_lab(row)
            st.caption(f"Quick check: Pitch mix {pitch_mode} • {pitch_mix} | xSLG: {xslg:.3f} | Trend: {recent} | Weather: {weather}")
...
base_tabs = ["JR HR Board", "Top 12", "Top HR Targets", "Pitchers to Target", "HR Combos", "Hits + Runs + RBIs", "Batter Breakdown", "Accuracy Tracker"]
