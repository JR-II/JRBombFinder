...
BF_FAST_HTTP_TIMEOUT = 6
BF_FAST_SAVANT_TIMEOUT = 7
...
def read_html_best_table(urls: list[str], must_have_any: list[str]) -> pd.DataFrame:
    ...
    for url in urls:
        try:
            html = requests.get(url, headers=headers, timeout=BF_FAST_SAVANT_TIMEOUT).text
...
def fetch_weather_for_park(home_team_abbr: str):
    ...
    try:
        resp = requests.get(url, timeout=BF_FAST_HTTP_TIMEOUT)
...
def fetch_people_stats(person_ids_tuple: tuple, group: str):
    ...
    for chunk in chunked(person_ids, 40):
        ...
        try:
            resp = requests.get("https://statsapi.mlb.com/api/v1/people", params=params, timeout=BF_FAST_HTTP_TIMEOUT)
            resp.raise_for_status()
            data = resp.json()
        except Exception:
            continue
