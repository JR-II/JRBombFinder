...
with c4:
    ...
    if confirmed_locked > 0:
        st.caption(f"Projected teams stay live â¢ confirmed teams pregame-rebuild on update â¢ locked confirmed rows: {confirmed_locked}")
    else:
        st.caption(f"Projected teams live â¢ update rebuilds pregame confirmed locks â¢ last refresh: {datetime.now().strftime('%Y-%m-%d %I:%M %p')}")
...
for idx, game in enumerate(schedule, start=5):
    with tabs[idx]:
        st.subheader(f"{game['game_key']} â {format_game_time_et(game.get('game_time', ''))}")
        st.caption(
