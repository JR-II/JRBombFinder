...
def today_str() -> str:
    # BF Data tracker date is locked to the user's local Indiana time.
...
def build_visible_tracker_pool(df: pd.DataFrame, schedule: list[dict]) -> pd.DataFrame:
    ...
    if top12.empty:
        top12 = sort_for_hr(get_research_shortlist_pool(df)).head(12).copy()
        if not top12.empty and "Rank" not in top12.columns:
            top12 = add_rank_column(top12.reset_index(drop=True))
    if not top12.empty:
...
def get_player_hr_count_from_map(homer_map: dict, player_name: str, player_id=None) -> int:
    ...
    if player_id is not None and not pd.isna(player_id):
        pid_raw = str(player_id).replace(".0", "")
        best = max(best, safe_int(homer_map.get(pid_raw), 0))
    if not player_name:
        return best
    raw = str(player_name)
    best = max(best, safe_int(homer_map.get(raw), 0))
    norm = normalize_name(raw)
    best = max(best, safe_int(homer_map.get(norm), 0))
    for key, val in homer_map.items():
        if normalize_name(key) == norm:
            best = max(best, safe_int(val, 0))
    return int(best)


@st.cache_data(ttl=15)
def fetch_today_player_hr_counts(player_ids_tuple: tuple) -> dict:
    """Authoritative same-day HR totals from MLB StatsAPI game logs.

    Sums every gameLog split dated today in Indiana time. This prevents
    multi-HR days from being flattened into a simple yes/no result.
    """
    ids = []
    for x in player_ids_tuple:
        raw = str(x).replace(".0", "")
        if raw.isdigit():
            ids.append(raw)
    if not ids:
        return {}
    stats_map = fetch_people_stats(tuple(ids), "hitting")
    out = {}
    date_key = today_str()
    for pid, payload in stats_map.items():
        total = 0
        for g in payload.get("gamelog", []) or []:
            if str(g.get("date")) == date_key:
                total += safe_int((g.get("stat") or {}).get("homeRuns", 0), 0)
        out[str(pid)] = int(total)
    return out


def build_today_homer_master_map(schedule: list[dict], player_ids=None) -> dict:
    """Merged current-day HR map: live feed + boxscore + same-day game logs."""
    master = {}
    for game in schedule:
        game_pk = game.get("game_pk")
        if game_pk is None:
            continue
        game_map = get_boxscore_homers(game_pk)
        for key, val in game_map.items():
            master[key] = max(safe_int(master.get(key), 0), safe_int(val, 0))
    if player_ids:
        id_counts = fetch_today_player_hr_counts(tuple(player_ids))
        for key, val in id_counts.items():
            master[str(key)] = max(safe_int(master.get(str(key)), 0), safe_int(val, 0))
    return master
...
def add_live_homer_counts_to_board(df: pd.DataFrame, schedule: list[dict]) -> pd.DataFrame:
    ...
    player_ids = []
    if "Player ID" in out.columns:
        player_ids += out["Player ID"].dropna().astype(str).tolist()
    if "player_id" in out.columns:
        player_ids += out["player_id"].dropna().astype(str).tolist()

    master_map = build_today_homer_master_map(schedule, tuple(player_ids))

    def _row_hr_count(row):
        pid = row.get("Player ID", row.get("player_id", None))
        return get_player_hr_count_from_map(master_map, row.get("Player"), pid)

    out["Actual HR Today"] = out.apply(_row_hr_count, axis=1).astype(int)
    return out
...
def auto_update_tracker_results(tracker: pd.DataFrame, schedule: list[dict]):
    ...
    for game in schedule:
        ...
        homer_map = dict(master_homer_map)
        game_map = get_boxscore_homers(game_pk)
        for k, v in game_map.items():
            homer_map[k] = max(safe_int(homer_map.get(k), 0), safe_int(v, 0))
